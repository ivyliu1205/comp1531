...
@login_required
def book(self):
    healthProvider = system.get_healthProvider(self)
    if healthProvider is None:
        abort(404)
    if request.method == 'POST':
        date_format = "%Y-%m-%d"
        booking_date = datetime.strptime(request.form['booking_date'], date_format)
        if 'check' in request.form:
            ...
            location = Location(request.form['where'])
            booking = system.make_booking(system.get_user_by_id(current_user.get_id()), diff, healthProvider, location)
            return render_template('booking_confirm.html', booking=booking)
    return render_template('booking_form.html', healthProvider=healthProvider)
...
def bookings(rego):
    ...
    healthProvider = system.get_healthProvider(self)
    bookings = healthProvider.get_bookings()
